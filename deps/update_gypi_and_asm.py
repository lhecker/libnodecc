...
#                                            #
#  2014, Leonard Hecker:                     #
#  Changed subpaths from src/ to boringssl/  #
#                                            #
##############################################


"""Enumerates the BoringSSL source in boringssl/ and generates two gypi files:
...
def WriteAsmFiles(perlasms):
  ...
  for osarch in OS_ARCH_COMBOS:
    ...
    key = (osname, arch)
    outDir = 'boringssl_%s-%s' % key

    for perlasm in perlasms:
      filename = os.path.basename(perlasm['input'])
      output = perlasm['output']
      if not output.startswith('boringssl'):
        raise ValueError('output missing boringssl: %s' % output)
...
def main():
  crypto_c_files = FindCFiles(os.path.join('boringssl', 'crypto'), NoTests)
  ssl_c_files = FindCFiles(os.path.join('boringssl', 'ssl'), NoTests)
  ...
  test_c_files = FindCFiles(os.path.join('boringssl', 'crypto'), OnlyTests)
